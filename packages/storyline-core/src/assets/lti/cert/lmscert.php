...
<img src="http://www.imsglobal.org/images/imslogo.png" align="right" border="0"/>
</a>
</p>
<?php
...
if ( isset($resource_link_title) ) {
    if ($resource_link_title == strip_tags($resource_link_title)) {
        mark_pass("1.2", "Recommended resource_link_title present");
    } else {
        mark_fail("1.2", "looks like resource_link_title contains HTML");
    }
}
if ( isset($resource_link_description) ) {
    if ($resource_link_description == strip_tags($resource_link_description)) {
        mark_pass("1.3", "Optional resource_link_description present");
    } else {
        mark_fail("1.3", "looks like resource_link_description contains HTML");
    }
}
...
if ( isthere($_REQUEST['lis_person_sourcedid']) ) {
    mark_pass("4.9", "Sends lis_person_sourcedid");
}
...
if ( isthere($_REQUEST['context_label']) ) {
    if ($_REQUEST['context_label'] == strip_tags($_REQUEST['context_label'])) {
        mark_pass("5.2", "Sends context_label");
    } else {
        mark_fail("5.2", "looks like context_label contains HTML");
    }
}
if ( isthere($_REQUEST['context_title']) ) {
    if ($_REQUEST['context_title'] == strip_tags($_REQUEST['context_title'])) {
        mark_pass("5.3", "Sends context_title");
    } else {
        mark_fail("5.3", "looks like context_title contains HTML");
    }
}
...
if ( isthere($_REQUEST['lis_course_offering_sourcedid']) ) {
    mark_pass("5.7", "Sends lis_course_offering_sourcedid");
}
if ( isthere($_REQUEST['lis_course_section_sourcedid']) ) {
    mark_pass("5.8", "Sends lis_course_section_sourcedid");
}


// 6.0 Consumer support
if ( isthere($_REQUEST['tool_consumer_instance_guid']) ) {
    mark_pass("6.1", "Sends tool_consumer_instance_guid");
}
if ( isthere($_REQUEST['tool_consumer_instance_name']) ) {
    if ($_REQUEST['tool_consumer_instance_name'] == strip_tags($_REQUEST['tool_consumer_instance_name'])) {
        mark_pass("6.2", "Sends tool_consumer_instance_name");
    } else {
        mark_fail("6.2", "looks like tool_consumer_instance_name contains HTML");
    }
}
if ( isthere($_REQUEST['tool_consumer_instance_description']) ) {
    if ($_REQUEST['tool_consumer_instance_description'] == strip_tags($_REQUEST['tool_consumer_instance_description'])) {
        mark_pass("6.3", "Sends tool_consumer_instance_description");
    } else {
        mark_fail("6.3", "looks like tool_consumer_instance_description contains HTML");
    }
}
if ( isthere($_REQUEST['tool_consumer_instance_contact_email']) ) {
    mark_pass("6.4", "Sends tool_consumer_instance_contact_email");
}


// 7.0 Consistency / Mapping
...
foreach($mapping as $k1 => $value ) {
  ...
  // echo("</pre>");
        ...
        if ( $value[2] == $check[2] and $value[0] != $check[0] ) {
            mark_pass("7.4", "Support for more than one resource_link_id from a context_id");
        }
        if ( $value[0] == $check[0] and $value[2] != $check[2] ) {
            mark_fail("7.5", "A resource_link_id must always come from the same context_id");
        }
        if ( $value[1] == $check[1] and $value[3] != $check[3] ) {
            mark_fail("7.6", "A user_id must not switch roles in a context_id");
        }
    }
}

// 8.0 Launch materials
if ( isthere($_REQUEST['launch_presentation_locale']) ) mark_pass("8.1", "launch_presentation_locale");

$target = $_REQUEST['launch_presentation_document_target'];
if ( isset($target) ) {
    if ( $target == 'frame' or $target == 'iframe' or $target == 'window' ) {
        mark_pass("8.2", "launch_presentation_document_target");
    } else {
        mark_fail("8.2", "launch_presentation_document_target");
    }
}
...
if ( isset($_REQUEST['launch_presentation_height']) ) {
    ...
    if ( $height > 0 ) {
        mark_pass("8.3", "launch_presentation_height");
    } else {
        mark_fail("8.3", "launch_presentation_height");
    }
}
...
if ( isset($_REQUEST['launch_presentation_width']) ) {
    ...
    if ( $width > 0 ) {
        mark_pass("8.4", "launch_presentation_width");
    } else {
        mark_fail("8.4", "launch_presentation_width");
    }
}

$css = $_REQUEST['launch_presentation_css_url'];
error_log($css);
if ( isset($css) ) {
    if (do_get_request($css) !== FALSE) {
        mark_pass("8.5", "launch_presentation_css_url");
    } else {
        mark_fail("8.5", "launch_presentation_css_url");
    }
}

// 9.0  Outcomes
if ( isset($_REQUEST['lis_result_sourcedid']) && isset($_REQUEST['lis_outcome_service_url']) ) {
   mark_pass("9.1", "Sends lis_result_sourcedid and lis_outcome_service_url");
   ...
   print 'here to run the 9.x tests of that service.</a>.</p>'."\n";
}

if (!empty($resource_link_id) && isset($roles)) {
    $therole = instructor_or_learner($roles);
    if ($therole !== FALSE) {
        $hasOutcomesURL = isthere($_REQUEST['lis_outcome_service_url']);
        $isInstructor = $therole == 'Instructor';
        $isLearner = $therole == 'Learner';
        if (!isset($outcomes[$resource_link_id])) {
            $outcomes[$resource_link_id] = array($hasOutcomesURL, $isInstructor, $isLearner);
        } else {
            $thisoutcome = $outcomes[$resource_link_id];
            if ($thisoutcome[0] != $hasOutcomesURL) {
                mark_fail('9.9', 'lis_outcome_service_url');
            } else {
                $thisoutcome[1] = $thisoutcome[1] || $isInstructor;
                $thisoutcome[2] = $thisoutcome[2] || $isLearner;
                if ($thisoutcome[0] && $thisoutcome[1] && $thisoutcome[2]) {
                    mark_pass('9.9', 'lis_outcome_service_url');
                }
                $outcomes[$resource_link_id] = $thisoutcome;
            }
        }
    }
}
