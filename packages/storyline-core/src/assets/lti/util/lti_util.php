    ...
    $org_id = false, $org_desc = false)
{
  ...
  function postLaunchHTML($newparms, $endpoint, $submit_text = false, $debug=false, $iframeattr=false) {
    global $last_base_string;
...
function post_socket_xml($endpoint, $data, $moreheaders=false) {
  ...
  // echo("\n"); echo($headers); echo("\n");
    ...
    try {
      ...
      $fp = fsockopen($hostprefix . $url['host'], $url['port'], $errno, $errstr, 30);
      if($fp) {
        fputs($fp, $headers);
        $result = '';
