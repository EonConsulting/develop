...
class SimpleHtmlDom
{
    ...
    function __construct($str=null, $lowercase=true, $forceTagsClosed=true, $targetCharset=HtmlHandler::DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=HtmlHandler::DEFAULT_BR_TEXT, $defaultSpanText=HtmlHandler::DEFAULT_SPAN_TEXT)
    {
        if ($str)
        {
    ...
    function load($str, $lowercase=true, $stripRN=true, $defaultBRText=HtmlHandler::DEFAULT_BR_TEXT, $defaultSpanText=HtmlHandler::DEFAULT_SPAN_TEXT)
    {
        global $debugObject;
        ...
        while ($this->parse());
        // end
        $this->root->_[HtmlHandler::HDOM_INFO_END] = $this->cursor;
        $this->parseCharset();
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=HtmlHandler::DEFAULT_BR_TEXT, $defaultSpanText=HtmlHandler::DEFAULT_SPAN_TEXT)
    {
        $this->clear();
    ...
    protected function parse()
    {
        ...
        ++$this->cursor;
        $node->_[HtmlHandler::HDOM_INFO_TEXT] = $s;
    ...
    protected function readTag()
    {
        ...
        if ($this->char==='/')
        {
            ...
            if ($parentLower!==$tagLower)
            {
                ...
                {
                    $this->parent->_[HtmlHandler::HDOM_INFO_END] = 0;
                    ...
                    if (strtolower($this->parent->tag)!==$tagLower) {
                        ...
                        $this->parent->_[HtmlHandler::HDOM_INFO_END] = $this->cursor;
        ...
        $node = new SimpleHtmlDomNode($this);
        $node->_[HtmlHandler::HDOM_INFO_BEGIN] = $this->cursor;
        ++$this->cursor;
        ...
        if (isset($tag[0]) && $tag[0]==='!') {
            $node->_[HtmlHandler::HDOM_INFO_TEXT] = '<' . $tag . $this->copyUntilChar('>');

            if (isset($tag[2]) && $tag[1]==='-' && $tag[2]==='-') {
                $node->nodetype = HtmlHandler::HDOM_TYPE_COMMENT;
                $node->tag = 'comment';
            } else {
                $node->nodetype = HtmlHandler::HDOM_TYPE_UNKNOWN;
                $node->tag = 'unknown';
            }
            if ($this->char==='>') $node->_[HtmlHandler::HDOM_INFO_TEXT].='>';
        ...
        if ($pos=strpos($tag, '<')!==false) {
            $tag = '<' . substr($tag, 0, -1);
            $node->_[HtmlHandler::HDOM_INFO_TEXT] = $tag;
            $this->linkNodes($node, false);
        ...
        if (!preg_match("/^[\w-:]+$/", $tag)) {
            $node->_[HtmlHandler::HDOM_INFO_TEXT] = '<' . $tag . $this->copyUntil('<>');
            if ($this->char==='<') {
                $this->linkNodes($node, false);
                return true;
            }
        ...
        // begin tag
        $node->nodetype = HtmlHandler::HDOM_TYPE_ELEMENT;
        $tagLower = strtolower($tag);
        ...
        do
        {
            ...
            if ($this->pos>=$this->size-1 && $this->char!=='>') {
                $node->nodetype = HtmlHandler::HDOM_TYPE_TEXT;
                $node->_[HtmlHandler::HDOM_INFO_END] = 0;
                $node->_[HtmlHandler::HDOM_INFO_TEXT] = '<'.$tag . $space[0] . $name;
                $node->tag = 'text';
                $this->linkNodes($node, false);
                return true;
            }
            ...
            if ($this->doc[$this->pos-1]=='<') {
                ...
                $node->attr = array();
                $node->_[HtmlHandler::HDOM_INFO_END] = 0;
                $node->_[HtmlHandler::HDOM_INFO_TEXT] = substr($this->doc, $beginTagPos, $this->pos-$beginTagPos-1);
                $this->pos -= 2;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $this->linkNodes($node, false);
                return true;
            ...
            if ($name!=='/' && $name!=='') {
                ...
                if ($this->char==='=') {
                    ...
                    $node->_[HtmlHandler::HDOM_INFO_QUOTE][] = HtmlHandler::HDOM_QUOTE_NO;
                    $node->attr[$name] = true;
                    if ($this->char!='>') $this->char = $this->doc[--$this->pos]; // prev
                }
                $node->_[HtmlHandler::HDOM_INFO_SPACE][] = $space;
                $space = array($this->copySkip($this->tokenBlank), '', '');
            }
            else
                break;
        ...
        $this->linkNodes($node, true);
        $node->_[HtmlHandler::HDOM_INFO_ENDSPACE] = $space[0];

        // check self closing
        if ($this->copyUntilCharEscape('>')==='/')
        {
            $node->_[HtmlHandler::HDOM_INFO_ENDSPACE] .= '/';
            $node->_[HtmlHandler::HDOM_INFO_END] = 0;
        }
        else
        {
        ...
        if ($node->tag == "br")
        {
            $node->_[HtmlHandler::HDOM_INFO_INNER] = $this->defaultBrText;
        }
    ...
    protected function parseAttr($node, $name, &$space)
    {
        ...
        switch ($this->char) {
            case '"':
                $node->_[HtmlHandler::HDOM_INFO_QUOTE][] = HtmlHandler::HDOM_QUOTE_DOUBLE;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $node->attr[$name] = $this->restoreNoise($this->copyUntilCharEscape('"'));
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                break;
            case '\'':
                $node->_[HtmlHandler::HDOM_INFO_QUOTE][] = HtmlHandler::HDOM_QUOTE_SINGLE;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $node->attr[$name] = $this->restoreNoise($this->copyUntilCharEscape('\''));
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                break;
