...
use EONConsulting\Core\Services\Elastic\Classes\Search;
use EONConsulting\Core\Services\Elastic\Index;
...
class Query
{
    ...
    public function where($name, $operator = "=", $value = NULL)
    {
        ...
        if ($this->is_callback_function($name)) {
            $name($this);
            return $this;
        }
    ...
    protected function getAll($result = [])
    {
        ...
        foreach (array_get($result, 'hits.hits') as $row)
        {
            $hits[] = [
                '_index' => array_get($row, '_index'),
                '_type' => array_get($row, '_type'),
                '_id' => array_get($row, '_id'),
                '_score' => array_get($row, '_score'),
                '_source' => array_get($row, '_source'),
            ];
        }

        $collection = new Collection($hits);

        $collection->total = array_get($result, 'hits.total');
        $collection->max_score = array_get($result, 'hits.max_score');
        $collection->took = array_get($result, 'took');
        $collection->timed_out = array_get($result, 'timed_out');
        $collection->scroll_id  = array_get($result, '_scroll_id');
        $collection->shards = (object) array_get($result, '_shards');
