...
if ( isset($testid) and isset($tool_tests[$testid]) ) {
   ...
   $content = postLaunchHTML($parms, $endpoint, "Press to Launch", $dodebug);
   print($content);
   exit;
}
...
<center>
<?php
if ($_SESSION['use_window']) {
?>
<form action="toolcert.php"" method="GET" target="toolcert">
<input type="hidden" name="testid" value="<?php echo($testshow); ?>" />
<?php
if ( isset($_GET['debug']) ) echo('<input type="hidden" name="debug" value="yes" />');
?>
<input type="submit" value="Launch test in new window" />
</form>
<?php
} else {
?>
